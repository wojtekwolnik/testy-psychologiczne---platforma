...
const UserManagement: React.FC = () => {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">{users.map(user => (
            <tr key={user.id}>
              ...
              <td className="p-4"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.role === UserRole.Admin ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>{user.role}</span></td>
      ...
      {isDeleteModalOpen && selectedUser && <ActionConfirmModal isOpen={isDeleteModalOpen} title="Potwierdź usunięcie" message={`Czy na pewno chcesz usunąć użytkownika ${selectedUser.fullName}?`} onConfirm={() => handleDelete(selectedUser.id)} onCancel={() => setIsDeleteModalOpen(false)} />}
