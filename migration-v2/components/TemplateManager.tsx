...
import { fetchPdfTemplates, deletePdfTemplate, fetchTests } from '@/app/actions/testActions';
    ...
    return (
        <>
            <div className="p-8 max-w-7xl mx-auto">
                            ...
                            <tbody>
                                {isLoading ? (<tr><td colSpan={4} className="p-8 text-center">Ładowanie...</td></tr>)
                                    : error ? (<tr><td colSpan={4} className="p-8 text-center text-[var(--error-color)]">{error}</td></tr>)
                                        : templates.length === 0 ? (
                                            <tr><td colSpan={4} className="p-8 text-center opacity-60">Brak zdefiniowanych szablonów.</td></tr>
                                            ...
                                            <tr key={template.id} className="border-b border-[var(--border-color)] hover:bg-[var(--background-color)]">
                                                ...
                                                <td className="p-4 flex gap-2">
                                                    <button onClick={() => router.push(`/admin/template/edit/${template.id}`)} className="p-2 opacity-70 hover:opacity-100" title="Edytuj"><EditIcon /></button>
                ...
                <button onClick={() => router.push('/admin/dashboard')} className="mt-8 px-5 py-2 bg-slate-200 text-slate-800 font-semibold rounded-lg hover:bg-slate-300">
